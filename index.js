...
/**
 * @description Search for movies, actors, directors, or genres based on the search term.
 * @name GET /search/:searchTerm
 * @example
 ...
 * [
 *   {
 *     "type": "movie", // Type of the search result (movie, actor, director, genre)
 *     "title": "Movie Title", // Movie title
 *     "description": "Description of the movie", // Short description
 *     "genre": {
 *       "_id": "60c72b2f9b1d4c3f9cb47c80", // Genre ID (ObjectID as string)
 *       "name": "Genre Name" // Genre name
 *     },
 *     "director": {
 *       "_id": "60c72b2f9b1d4c3f9cb47c81", // Director ID (ObjectID as string)
 *       "name": "Director Name" // Director name
 *     },
 *     "image": "image/path.jpg", // Path or URL to the movie poster
 *     "releaseYear": 2022, // Year of release (only for movies)
 *     "actors": [
 *       {
 *         "_id": "60c72b2f9b1d4c3f9cb47c82", // Actor ID (ObjectID as string)
 *         "name": "Actor Name", // Actor's name
 *         "role": "Main Character" // Role played
 *       }
 *     ],
 *     "_id": "60c72b2f9b1d4c3f9cb47c7e" // Unique identifier (ObjectID as string)
 *   }
 * ]
...
/**
 * @summary Automatically serves all static files from the "public" folder.
 *
 * @description This route handles serving all static files from the "public" folder when requested.
 *
 * @function
 */
 ...
 * "Welcome to DojoDB - Let's kick things off!"
 * @returns {string} 200 - A welcome message: "Welcome to DojoDB - Let's kick things off!"
 * @async
 */
app.get('/', (req, res) => {
 ...
 * [
 *   {
 *     "type": "movie", // Type of the search result (movie)
 ...
 * @name GET /movies/release-year/:year
 * @example
 * Authentication: Bearer token (JWT)
 * @example
 * Request data format:
...
/**
 * @description Get data about an actor by name.
 ...
 * Response data format:
 * {
 *   "type": "actor", // Type of the search result (actor)
 *   "name": "Actor Name", // Actor's name
 *   "roles": [
 *     {
 *       "title": "Movie Title", // Title of the movie the actor starred in
 *       "role": "Main Character", // Role of the actor in the movie
 *       "releaseYear": 2022, // Year of the movie's release
 *       "description": "Movie Description", // Description of the movie
 *       "genre": "Genre Name", // Genre of the movie
 *       "director": "Director Name", // Director of the movie
 *       "image": "image/path.jpg", // Path or URL to the image of the movie
 ...
 * @returns {Object} 200 - Actor details, including a list of movies they starred in.
 * @returns {Error} 404 - If no actor is found with the given name.
 * @returns {Error} 500 - If an internal server error occurs while fetching actor data.
 * @async
 */
 ...
 * Response data format:
 * {
 *   "type": "genre", // Type of the search result (genre)
 *   "name": "Genre Name", // Genre name
 ...
 * Response data format:
 * {
 *   "username": "kung_fu", // Username of the user
 *   "email": "kung@example.com", // User's email address
 *   "birthday": "1990-01-01", // User's birthday (if provided)
 *   "favourites": [ // List of favorite movie objects (IDs)
 *     "60c72b2f9b1d4c3f9cb47c7e", // Movie ID
 *     "60c72b2f9b1d4c3f9cb47c80"  // Another Movie ID
 *   ]
 * }
 ...
 * {
 *   "username": "new_kung",
 *   "email": "new_kung@example.com",
 *   "password": "password123",
 *   "birthday": "1992-04-12" // Optional
 * }
 * @example
 * Response data format:
 * {
 ...
 *   "user": {
 *     "_id": "60c72b2f9b1d4c3f9cb47c7e", // Unique identifier (ObjectID as string)
 *     "username": "new_kung", // New user's username
 *     "email": "new_kung@example.com", // New user's email
 *     "birthday": "1992-04-12" // New user's birthday
 *   },
 ...
 * Request data format:
 * {
 *   "newUsername": "updated_kung", // Optional new username
 *   "newEmail": "updated_kung@example.com", // Optional new email
 *   "newPassword": "newpassword123", // Optional new password
 *   "newBirthday": "1992-04-12", // Optional new birthday
 *   "favourites": [ // Optional favourite movies to add or remove
 ...
 *   "updatedFields": {
 *     "username": "updated_kung updated successfully",
 *     "favourites (added movies)": "added movie successfully"
 *   },
 *   "user": {
 *     "username": "updated_kung", // Updated username
 *     "email": "updated_kung@example.com", // Updated email
 *     "birthday": "1992-04-12", // Updated birthday
 *     "favourites": [ "60c72b2f9b1d4c3f9cb47c7e" ] // Updated favourites
 *   }
 * }
 ...
 * @name PUT /users/:username/favourites/:movieID
 * @example
 * Authentication: Bearer token (JWT)
 * @example
 * Request data format:
 ...
 * Response data format:
 * {
 *   "message": "Movie 'Movie Title' with the ID '60c72b2f9b1d4c3f9cb47c7e' added to favourites.", // Success message
 *   "username": "kung_fu", // Username of the authenticated user
 *   "favourites": [ // List of favourite movies
 *     {
 *       "movieId": "60c72b2f9b1d4c3f9cb47c7e", // Movie ID
 *       "title": "Movie Title" // Movie title
 *     },
 *     ...
 *   ]
 ...
 * @name DELETE /users/:username/favourites/:movieID
 * @example
 * Authentication: Bearer token (JWT)
 * @example
 * Request data format:
 ...
 * Response data format:
 * {
 *   "message": "Movie with the ID '60c72b2f9b1d4c3f9cb47c7e' removed from favourites.", // Success message
 *   "username": "kung_fu", // Username of the authenticated user
 *   "favourites": [ // Updated list of favourite movies
 *     {
 *       "movieId": "60c72b2f9b1d4c3f9cb47c7f", // Movie ID
 ...
 * Response data format:
 * {
 *   "message": "User with the username 'kung_fu' has been removed.", // Success message
 *   "username": "kung_fu" // Username of the deleted user
 * }
